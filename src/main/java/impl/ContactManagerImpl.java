...
public class ContactManagerImpl implements ContactManager, Serializable {
        ...
        try {
            ...
            if(file.exists() && file.isFile()){
               input = new ObjectInputStream(new FileInputStream(FILE));
                //Deserialize the objects - Create data structures for the file content
                contacts = (TreeSet<Contact>)input.readObject();
                meetings = (ArrayList<Meeting>)input.readObject();
    ...
    public int addFutureMeeting (Set<Contact> contacts, Calendar date) throws IllegalArgumentException, NullPointerException {
        ...
        Meeting futureMeeting = new FutureMeetingImpl(meetingId.nextValue(), date, contacts);
     ...
     */
    private Meeting searchForMeeting(int id) {
        Iterator<Meeting> iterate = meetings.iterator();
        Meeting meeting = null;
        boolean found = false;
        while(iterate.hasNext() && found == false) {
     ...
     */
    private List<Meeting> sortMeetingByDate(List<Meeting> meetings) {
        for(int j = 1 ; j < meetings.size() ; j ++) {
            for(int i = meetings.size()-1; i>=j; i--) {
                ...
                if(next.before(previous)) {
                    ...
                    meetings.set(i-1, previousMeeting);
                    meetings.set(i, nextMeeting);
                }
            }
        }
        return meetings;
    ...
    public List<Meeting> getMeetingListOn (Calendar date) throws NullPointerException {
        ...
        //Convert Calendar date to local date time in order to have access to individual date components
        LocalDateTime localDateTime = calendarToLDT(date);
        //Create a list ot hold the meetings that satisfy the criterion
        List<Meeting> meetingList = new ArrayList<>();
        //Go through all the meetings and select the ones that satisfy the criterion
        Iterator<Meeting> iterate = meetings.iterator();
        while(iterate.hasNext()) {
            Meeting meeting = iterate.next();
            //Get the date of the meeting and convert it to a local date time to compare it with the date
            //give as a parameter
            ...
            if(year == localDateTime.getYear() && month == localDateTime.getMonth().getValue()
                ...
                while(iterator.hasNext() && !found) {
                    ...
                    LocalDateTime lDT = calendarToLDT(dayOfMeeting);
                    int comparingHours = lDT.getHour();
                    int comparingMinutes = lDT.getMinute();
    ...
    private LocalDateTime calendarToLDT(Calendar date) {
    ...
    public int addNewPastMeeting(Set<Contact> contacts, Calendar date, String text) throws IllegalArgumentException,
        ...
        if(date.after(Calendar.getInstance())) {
            throw new IllegalArgumentException("The date provided is in the future");
        }
        Iterator<Contact> iterate = contacts.iterator();
        while(iterate.hasNext()) {
            Contact contact = iterate.next();
        ...
        Meeting pastMeeting = new PastMeetingImpl(meetingId.nextValue() , date, contacts, text);
    ...
    public PastMeeting addMeetingNotes(int id, String text) throws IllegalArgumentException,
        ...
        if(meeting instanceof PastMeeting) {
            meeting = new PastMeetingImpl(id, meeting.getDate(), meeting.getContacts(), text);
            meetings.set(i,meeting);
            return (PastMeeting)meeting;
        }

        int j = addNewPastMeeting(meeting.getContacts(), meeting.getDate(), text);
